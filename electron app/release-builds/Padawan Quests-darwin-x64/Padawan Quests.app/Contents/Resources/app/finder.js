...
function openCWindow(urlLink) {
  debugger;
  window.open(urlLink, '_blank', 'nodeIntegration=no')
}
...
function createIssuePost(issueData) {
  ...
  let htmlContent = `<div class='post'>
    <h3 class = "issueRepo">
      <a data-ref = "${repositoryUrl}" onclick = "openCWindow(this.dataset.ref)" href='#'>${repoName}</a>
    </h3>
    <h3 class = "issueHeading">
      <a data-ref = "${htmlUrl}" onclick = "openCWindow(this.dataset.ref)" href='#'> ${issueData.title} </a>
    </h3>
